...
cap=cv2.VideoCapture("proiect.mp4")
...
#Lines
line_up=int(2.8*(h/4))
line_down=int(4.8*(h/5))

up_limit=int(2.4*(h/4))
                ...
                if cy in range(up_limit,down_limit):
                    for i in cars:
                        if abs(x - i.getX()) <= w and abs(y - i.getY()) <= h:
                            ...
                            if i.l_going_UP(line_down,line_up)==True:
                                cnt_up+=1
                                print("ID:",i.getId(),'crossed going up at', time.strftime("%c"))
                            elif i.l_going_DOWN(line_up,line_down)==True:
                                cnt_up-=1
                                print("ID:",i.getId(),'crossed going up at', time.strftime("%c"))
                            break
                        if i.getState()=='1':
